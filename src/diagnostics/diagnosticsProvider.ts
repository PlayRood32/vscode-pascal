...
export class DiagnosticsProvider implements vscode.Disposable {
    ...
    constructor(symbolCache: SymbolCache) {
        ...
        Logger.log('✅ DiagnosticsProvider initialized');
    }
    ...
    private onDocumentChange(event: vscode.TextDocumentChangeEvent) {
        ...
        if (!config.get<boolean>('syntaxCheckOnChange', true)) return;
    ...
    private onDocumentSave(document: vscode.TextDocument) {
        ...
        const config = vscode.workspace.getConfiguration('pascal');
        if (config.get<boolean>('syntaxCheckOnSave', true)) {
    ...
    public async checkSyntax(document: vscode.TextDocument): Promise<void> {
        const config = vscode.workspace.getConfiguration('pascal');
        const compiler = config.get<string>('compilerPath', 'fpc');
        ...
        try {
            ...
            Logger.log(`Syntax check error: ${error.message}`);
        }
    }
    ...
    private async runCompilerCheck(
        ...
        return new Promise((resolve) => {
            ...
            const options = config.get<string[]>('compilerOptions', []).join(' ');
    ...
    private performSemanticAnalysis(document: vscode.TextDocument): vscode.Diagnostic[] {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (this.shouldHaveSemicolon(trimmed, lines, i)) {
                const diagnostic = new vscode.Diagnostic(
                    new vscode.Range(i, line.length, i, line.length),
                    'Missing semicolon',
            ...
            if (/write(?:ln)?|read(?:ln)?/i.test(trimmed) && !/WriteLn|ReadLn|Write|Read/.test(line)) {
                ...
                if (match) {
                    ...
                    const diagnostic = new vscode.Diagnostic(
                        new vscode.Range(i, start, i, start + match[0].length),
                        'Inconsistent casing for I/O function',
    ...
    private shouldHaveSemicolon(line: string, lines: string[], index: number): boolean {
    ...
    private hasMatchingEnd(lines: string[], beginIndex: number): boolean {
        let depth = 1;
        
        for (let i = beginIndex + 1; i < lines.length && i < beginIndex + 100; i++) {
            const line = lines[i].toLowerCase();
            if (line.includes('begin')) depth++;
            if (line.includes('end')) depth--;
    ...
    private getModeFlag(): string {
        const config = vscode.workspace.getConfiguration('pascal');
        const mode = config.get<string>('compilerMode', 'FPC');
