...
class Lexer {
    constructor(input) {
        ...
        this.keywords = new Set([
            'program', 'unit', 'uses', 'begin', 'end', 'var', 'const', 'type',
            'procedure', 'function', 'if', 'then', 'else', 'while', 'do',
            'for', 'to', 'downto', 'repeat', 'until', 'case', 'of', 'record',
            'class', 'interface', 'implementation', 'initialization', 'finalization'
        ]);
        this.input = input;
    }
    tokenize() {
    ...
    tokenizeOperatorOrPunctuation() {
        ...
        const value = this.input.substring(startPos, this.pos);
        this.tokens.push({
            type: value.length > 1 ? types_1.TokenType.Operator : types_1.TokenType.Punctuation,
            value,
            line: startLine,
            column: startColumn,
            startOffset: startPos,
    ...
    isAlpha(ch) { return /[a-zA-Z]/.test(ch); }
