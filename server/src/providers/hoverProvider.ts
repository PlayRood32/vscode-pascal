import { Hover, HoverParams, MarkupKind, Position, Range } from 'vscode-languageserver/node';
...
export class HoverProvider {
    ...
    public provide(params: HoverParams): Hover | null {
        ...
        if (builtInDocs[wordInfo.word]) {
            return {
                contents: { kind: MarkupKind.Markdown, value: builtInDocs[wordInfo.word] },
        ...
        if (symbol) {
            return {
                contents: { kind: MarkupKind.Markdown, value: `**${symbol.kind}** \`${symbol.name}\`` },
                range: wordInfo.range
            };
        }
